...
export async function POST(request: Request) {
  ...
  // Determine which plan the user wants: "personal" (default) or "family"
  let planType: "personal" | "family" = "personal";
  try {
    const body = await request.json();
    if (body?.planType === "family") planType = "family";
  } catch {
    // Body is optional; default to personal
  }
  ...
  if (!planId) {
    return NextResponse.json(
      { error: `El plan "${planType}" no está configurado en el servidor.` },
      { status: 500 }
    );
  }
  ...
  return NextResponse.json({ approveUrl, planType });
}
